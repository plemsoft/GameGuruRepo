...
void CallEncryptDecrypt(char* dwStringAddress, bool bEncryptIfTrue, bool bDoNotUseTempFolder)
{
   ...
   // for safety
    ...
    {
        if (strnicmp(dwStringAddress, pLocalTRemp, strlen(pLocalTRemp)) == NULL)
        {
            // Delete temp decrypted file
            if (COREDoesFileExist(dwStringAddress)) DeleteFile(dwStringAddress);
    ...
    if (strlen(dwStringAddress) < 6) return;

    char checkForEncryptName[_MAX_PATH];
    char keyName[10] = "7yFkC30a";

    strcpy(checkForEncryptName, dwStringAddress);
    ...
    if ((checkForEncryptName[0] == '_' && checkForEncryptName[1] == 'e' && checkForEncryptName[2] == '_') || strstr(checkForEncryptName, "\\_e_"))
    {
        ...
        else
        {
            ...
            MessageBox(NULL, "This file is encrypted and cannot be loaded into GameGuru editor", "GAME GURU Error", MB_ICONEXCLAMATION);
        }
    }
    else
    {
    ...
    // where are we?
    //char pWhere[MAX_PATH];
    //GetCurrentDirectoryA(MAX_PATH, pWhere);
    ...
    if (COREDoesFileExist(pFilename))
    {
        ...
        if (bDoNotUseTempFolder == false)
        {
            ...
            if (bEncryptIfTrue == false)
            {
                ...
                strcpy(dwStringAddress, pFilename);
            }
        }
        else
        {
